import autogen  # Importing the autogen library to create and manage agents for autonomous task handling.
...
# Define the writing task for the financial report.
writing_tasks = [
...
# Create various assistant agents with specific roles.
...
# Exporter agent for potential file generation/export.
export_assistant = autogen.AssistantAgent(
    name="Exporter",
    llm_config=llm_config,
)

# Critic agent reviews the writer's work and initiates nested reviews.
...
# Additional reviewer agents to validate and improve content.
...
consistency_reviewer = autogen.AssistantAgent(
    ...
    llm_config=llm_config,
    system_message="You are a consistency reviewer, known for your ability to ensure that the written content is consistent throughout the report. Refer numbers and data in the report to determine which version should be chosen in case of contradictions. Make sure your suggestion is concise (within 3 bullet points), concrete and to the point. Begin the review by stating your role.",
)

textalignment_reviewer = autogen.AssistantAgent(
    name="Text_alignment_reviewer",
    llm_config=llm_config,
    system_message="You are a text data alignment reviewer, known for your ability to ensure that the meaning of the written content is aligned with the numbers written in the text. You must ensure that the text clearly describes the numbers provided in the text without contradictions. Make sure your suggestion is concise (within 3 bullet points), concrete and to the point. Begin the review by stating your role.",
)

completion_reviewer = autogen.AssistantAgent(
    name="Completion_Reviewer",
    llm_config=llm_config,
    system_message="You are a content completion reviewer, known for your ability to check that financial reports contain all the required elements. You always verify that the report contains: a news report about each asset, a description of the different ratios and prices, a description of possible future scenarios, a table comparing fundamental ratios and at least a single figure. Make sure your suggestion is concise (within 3 bullet points), concrete and to the point. Begin the review by stating your role.",
)
...
# Function to generate a reflection message for reviews.
def reflection_message(recipient, messages, sender, config):
    return f'''Review the following content. 
            \n\n {recipient.chat_messages_for_summary(sender)[-1]['content']}'''

# Define nested review chats initiated by the Critic agent.
review_chats = [
    {
...
# User proxy agent for handling user-triggered tasks.
user_proxy_auto = autogen.UserProxyAgent(
    ...
    code_execution_config={
        "last_n_messages": 3,
        "work_dir": "coding",  # Directory for code execution outputs.
        "use_docker": False,
    },
)
...
if hit_button is True:
    ...
    date_str = datetime.now().strftime("%Y-%m-%d")  # Current date for financial tasks.

    # Define financial tasks for the assistants.
    financial_tasks = [
    ...
    with st.spinner("Agents working on the analysis...."):
        # Initiate chats for the financial and research assistants, and writer.
    ...
    st.image("./coding/normalized_prices.png")  # Show generated image.
    st.markdown(chat_results[-1].chat_history[-1]["content"])  # Render final report in Markdown.
